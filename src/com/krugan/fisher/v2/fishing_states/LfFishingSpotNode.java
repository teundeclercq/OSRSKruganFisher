...
public class LfFishingSpotNode extends Node {
    ...
    public boolean isValid() {
        return !Inventory.isFull() && !main.isFishing && fishingLocation.fishingArea().contains(main.getLocalPlayer());
    }

    @Override
    public int execute() {
        state = "Looking for fishing spot";
        NPC fishingSpot = NPCs.closest(fs -> fs != null && fs.getName().contentEquals(fishes.fishingSpot().getFishingSpotName()) && fishingLocation.fishingArea().contains(fs));
